...
for ($idm650 = 1; $idm650 <= $nid650; $idm650++) {
    ...
    // 检查curl请求是否成功
    if ($re650 === false) {
        error_log("Failed to fetch data for channel: " . $cid650[$idm650 - 1][1]);
        continue;
    }
    
    $re650 = preg_replace_callback('/\\\\u([0-9a-f]{4})/i', 'replace_unicode_escape_sequence', $re650);// 適合php7
    
    $decodedData = json_decode($re650);
    
    // 检查JSON解码是否成功
    if ($decodedData === null) {
        error_log("Failed to decode JSON for channel: " . $cid650[$idm650 - 1][1]);
        continue;
    }
    
    // 安全检查数据结构
    if (!isset($decodedData->pageProps->channel->Schedule)) {
        error_log("No Schedule data found for channel: " . $cid650[$idm650 - 1][1]);
        continue;
    }
    
    $tuu650 = $decodedData->pageProps->channel->Schedule;
    $scheduleCount = is_countable($tuu650) ? count($tuu650) : 0;
    
    for ($i650 = 0; $i650 <= $scheduleCount - 2; $i650++) {
        // 安全检查数组元素
        if (!isset($tuu650[$i650]) || !isset($tuu650[$i650 + 1])) {
            continue;
        }
        
        $AirDateTime[$i650] = $tuu650[$i650]->AirDateTime ?? '';//2025-04-24T07:30:00Z
        $endDateTime[$i650] = $tuu650[$i650 + 1]->AirDateTime ?? '';//2025-04-24T07:30:00Z
        
        // 安全检查program对象
        $program = $tuu650[$i650]->program ?? null;
        $Title650[$i650] = $program->Title ?? '未知节目';
        $abtr650[$i650] = $program->Description ?? '暂无描述';
        
        // 确保时间数据有效
        if (!empty($AirDateTime[$i650]) && !empty($endDateTime[$i650])) {
            $startTime = str_replace('-', '', str_replace(':', '', str_replace('T', '', str_replace('Z', '', $AirDateTime[$i650])))) . ' +0000';
            $endTime = str_replace('-', '', str_replace(':', '', str_replace('T', '', str_replace('Z', '', $endDateTime[$i650])))) . ' +0000';
            
            $chn .= "<programme start=\"" . $startTime . "\" stop=\"" . $endTime . "\" channel=\"" . $cid650[$idm650 - 1][1] . "\">\n<title lang=\"zh\">" . str_replace('<', '', str_replace('>', '', $Title650[$i650])) . "</title>\n<desc lang=\"zh\">" . str_replace('<', '', str_replace('>', '', $abtr650[$i650])) . "</desc>\n</programme>\n";
        }
    }
}
...
for ($idm655 = 1; $idm655 <= $nid655; $idm655++) {
    ...
    // 检查curl请求是否成功
    if ($re655 === false) {
        error_log("Failed to fetch data for channel: " . $cid655[$idm655 - 1][1]);
        continue;
    }
    ...
    $decodedData655 = json_decode($re655);
    
    // 检查JSON解码是否成功和数据结构
    if ($decodedData655 === null || !isset($decodedData655->pageProps->channel->vod_channel_schedule->programs)) {
        error_log("No valid program data found for channel: " . $cid655[$idm655 - 1][1]);
        continue;
    }
    
    $tuu655 = $decodedData655->pageProps->channel->vod_channel_schedule->programs;
    $programCount = is_countable($tuu655) ? count($tuu655) : 0;

    for ($i655 = 0; $i655 <= $programCount - 2; $i655++) {
        // 安全检查数组元素
        if (!isset($tuu655[$i655]) || !isset($tuu655[$i655 + 1])) {
            continue;
        }

        $AirDateTime[$i655] = $tuu655[$i655]->p_start ?? 0;
        $endDateTime[$i655] = $tuu655[$i655 + 1]->p_start ?? 0;
        $Title655[$i655] = $tuu655[$i655]->title ?? '未知节目';
        $subtitle655[$i655] = $tuu655[$i655]->subtitle ?? '';
        $abtr655[$i655] = $tuu655[$i655]->vod_channel_description ?? '暂无描述';

        // 确保时间数据有效
        if ($AirDateTime[$i655] > 0 && $endDateTime[$i655] > 0) {
            $startTime = str_replace('-', '', str_replace(':', '', str_replace('T', '', str_replace('Z', '', date("YmdHis", $AirDateTime[$i655] / 1000))))) . ' +0000';
            $endTime = str_replace('-', '', str_replace(':', '', str_replace('T', '', str_replace('Z', '', date("YmdHis", $endDateTime[$i655] / 1000))))) . ' +0000';
            
            $chn .= "<programme start=\"" . $startTime . "\" stop=\"" . $endTime . "\" channel=\"" . $cid655[$idm655 - 1][1] . "\">\n<title lang=\"zh\">" . str_replace('<', '', str_replace('>', '', $Title655[$i655])) . "</title>\n<desc lang=\"zh\">" . str_replace('<', '', str_replace('>', '', $abtr655[$i655])) . "</desc>\n</programme>\n";
        }
    }
}
...
if (file_put_contents($fp, $chn) !== false) {
    echo "EPG文件生成成功: " . $fp . "\n";
} else {
    echo "EPG文件生成失败\n";
}
